...
export default class AutoSchema {
	...
	public static async updateSchema(pool: PGPool, dbName: string) {
		...
		try {
			...
			if (res.rows[0].db_auto_schema_update) {
				logger.info(
					`${dbName}: All Automated Schema Updates (if any) completed successfully`
				)
	...
	${result1.rows.map((contraint: any) => {
						return `CONSTRAINT ${contraint.constraint_name} ${contraint.constraint_type
							} (${contraint.column_name}) ${contraint.constraint_type === 'FOREIGN KEY'
	...
	${result2.rows.map((comments: any) => {
		...
		try {
			...
			res.rows.forEach(async (table: any) => {
				fs.writeFile(
					`../database/snapshots/${table.table_name}.sql`,
			...
			console.log('\x1b[32m%s\x1b[0m', `${dbName}: All Automated Table Snapshot Updates (if any) completed successfully`)
		} catch (err) {
			logger.error(`${dbName}: AutoSchema.updateSchema() Query: ${sql}`)
			logger.error(`${dbName}: AutoSchema.updateSchema() Query: ${err}`)
		}
